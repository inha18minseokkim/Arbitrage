...
class Determine:
    def __init__(self):
        ...
        self.price_data_ratio: List = [[np.array([]) for i in range(len(label.keys()))] for i in range(2)]#0번 양 1번 음
        self.price_data_std: List = [[0 for i in range(len(label.keys()))] for i in range(2)]#0번 양 1번 음
    ...
    def routineForThreeCoin(self,coin1,coin2,coin3): #SOL BNB BTC
        ...
        if ratio >= 0:
            ...
            idx = 1
        np.append(self.price_data_ratio[idx][label[coin1]], ratio)
        if len(self.price_data_ratio[idx][label[coin1]]) >= max_arr_size:
            self.price_data_std[idx][label[coin1]] = np.std(self.price_data_ratio[idx][label[coin1]])
            self.price_data_ratio[idx][label[coin1]] = self.price_data_ratio[idx][label[coin1]][-truncate_size:]
        ...
        if ratio >= 0 and ratio >= 0.075 + self.price_data_std[idx][label['SOL']]:
